...
output$scotland_results_table <- DT::renderDataTable({
  
  make_table(scotland_results)
...
scotland_forecast_f <- reactive({
  
  ts_data <- scotland_time_series()$ts
  
  fc <- Arima(ts_data, 
              order = c(input$scotland_ar_value, input$scotland_d_value, input$scotland_ma_value), 
              seasonal = c(1,0,1))
